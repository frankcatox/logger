...
var defaultLoggerMessageFormat = "[%level_string% %timestamp_format% %file%:%line%] %body%"
...
func colorMessageFormat(format string, loggerMsg *loggerMessage, attr color.Attribute) string {
	message := strings.Replace(format, "%timestamp%", color.New(color.FgCyan).Sprintf(strconv.FormatInt(loggerMsg.Timestamp, 10)), 1)
	message = strings.Replace(message, "%timestamp_format%", color.New(color.FgCyan).Sprintf(loggerMsg.TimestampFormat), 1)
	message = strings.Replace(message, "%millisecond%", color.New(color.FgCyan).Sprintf(strconv.FormatInt(loggerMsg.Millisecond, 10)), 1)
	message = strings.Replace(message, "%millisecond_format%", color.New(color.FgCyan).Sprintf(loggerMsg.MillisecondFormat), 1)
	message = strings.Replace(message, "%level%", color.New(color.FgBlack).Sprintf(strconv.Itoa(loggerMsg.Level)), 1)
	message = strings.Replace(message, "%level_string%", color.New(color.FgHiBlack).Sprintf(loggerMsg.LevelString), 1)
	message = strings.Replace(message, "%file%", color.New(color.FgMagenta).Sprintf(loggerMsg.File), 1)
	message = strings.Replace(message, "%line%", color.New(color.FgMagenta).Sprintf(strconv.Itoa(loggerMsg.Line)), 1)
	message = strings.Replace(message, "%function%", color.New(color.FgMagenta).Sprintf(loggerMsg.Function), 1)
	message = strings.Replace(message, "%body%", color.New(attr).Sprintf(loggerMsg.Body), 1)
	return message
}
