...
var levelColors = map[int]color.Attribute{
	...
	LOGGER_LEVEL_INFO:      color.FgGreen,   //blue
	LOGGER_LEVEL_DEBUG:     color.FgBlue,    //background blue
}
...
func (adapterConsole *AdapterConsole) Write(loggerMsg *loggerMessage) error {
	...
	if adapterConsole.config.JsonFormat == true {
		...
		if adapterConsole.config.Color {
			attr := adapterConsole.getColorByLevel(loggerMsg.Level)
			msg = colorMessageFormat(adapterConsole.config.Format, loggerMsg, attr)
		} else {
	...
	if adapterConsole.config.Color && adapterConsole.config.JsonFormat {
		colorAttr := adapterConsole.getColorByLevel(loggerMsg.Level)
...
func (adapterConsole *AdapterConsole) getColorByLevel(level int) color.Attribute {
